...
#     # ///
#
# `import_tools_to_wxo` parses those blocks out and writes one
# `<tool>_requirements.txt` per tool into src/flow_specs/, beside the compiled
# flow spec, then imports each tool with `-r` pointing at it. The file is only
# rewritten when the parsed block differs from what is already there: the
# server-side venv is cached by the requirements hash, so a needless rewrite
# risks a rebuild, and a rebuild is what makes a tool answer 424 while pip runs.
...
def parse_dependencies(source, tool_name=None):
    ...
    Scoped to the tool function bodies, NOT the whole file: a notebook holds
    logic blocks, prompt nodes and prose alongside the tool, and a `# ///` block
    belonging to one of those must not end up in another tool's requirements.
    With several @tool functions in one file, each declares its own.

    Args:
        source: The tool file's text.
        tool_name: Restrict to this function (or @tool(name=...)). None reads
            every @tool in the file, which is what a whole-file import needs,
            since one import ships them all.

    Returns:
        list[str]: The specs, in declaration order, deduplicated. Empty when no
        @tool declares a block -- normal for a tool with no third-party
        dependencies, and means no `-r` is passed for it.

    Falls back to scanning the whole file when the source will not parse (a
    partial cell, say), so a syntax error downgrades the scope rather than
    silently dropping the dependencies.
    """
    try:
    ...
    for node in ast.walk(tree):
        ...
        # The block sits inside the function body, so slice the function's own
        # source rather than the file's. `# ///` is a comment and therefore
        # absent from the AST, which is why this is a text scan of that slice.
...
def tools_in_source(source):
    ...
    otherwise the function name -- because that is the name the flow's tool node
    references, and so the name the requirements file should carry.

    A file may define several tools, each with its own `# ///` block; this keeps
    them apart rather than merging them into one set.
    """
    try:
        tree = ast.parse(source or "")
    except SyntaxError:
...
def tool_names_in_flow(aflow):
    ...
    available before compile() and without re-parsing JSON. Foreach/parallel
    scopes are Flow subclasses holding their own `nodes`, so this recurses.
...
def resolve_tool_sources(aflow, namespace):
    ...
    notebook imported under that name -- `from wxo_prize_details_node import
    fetch_url_data` puts it in the notebook's namespace, and inspect gives its
    file. Pass `globals()` from the calling cell as `namespace`.
    ...
    Returns:
        ...
        Unresolved names are the ones to warn about -- the flow will import with
...
### Where the generated requirements files live, alongside the compiled flow
### specs, so the two artefacts of an import sit together and are both reviewable
### in version control.
...
def extract_tool_source(source, tool_name=None):
    ...
    for node in ast.walk(tree):
        # marimo puts the shared imports in a `with app.setup:` block; a plain
        # module has them at top level. Take both, then filter below.
    ...
    # Only keep the schema classes the emitted tools actually mention, so an
    # unrelated class from elsewhere in the notebook does not ride along.
    body = "\n".join(tools)
    kept = [seg for name, seg in models if name in body]

    # Same for imports, but pruned per NAME rather than per statement: a notebook
    # writes `from typing import List, Union` for its own use, and keeping the
    # whole line because `List` is referenced would drag `Union` along unused.
    # Rebuilding each statement from only its used aliases avoids that, and drops
    # the statement entirely when nothing it binds is referenced -- which is what
    # keeps marimo, pandas, sqlalchemy and the rest out of the emitted file.
    referenced = body + "\n" + "\n".join(kept)

    # A name a tool imports INSIDE its own body needs no module-level import, and
    # hoisting one is actively harmful for a heavy dependency: a standalone tool
    # spawns a process per invocation, so a module-scope `import docling` pays the
    # torch/transformers load on every cold start, where a function-local one pays
    # it only when the function actually runs. Drop such names unless something
    # outside the function bodies -- a schema class, a decorator argument -- also
    # needs them.
    local_only = _locally_imported_names(tools)
    outside = "\n".join(kept) + "\n" + "\n".join(
        _strip_function_bodies(t) for t in tools
    )
...
def _is_app_decorator(node):
    ...
    return (
        ...
        and getattr(target.value, "id", None) == "app"
    )
        ...
        if isinstance(fn, (ast.FunctionDef, ast.AsyncFunctionDef)):
            for stmt in fn.body:
                for n in range(stmt.lineno, (stmt.end_lineno or stmt.lineno) + 1):
                    drop.add(n)
    return "\n".join(
        line for i, line in enumerate(lines, 1) if i not in drop
    )
...
def write_tool_source(source_text, tool_name, directory=REQUIREMENTS_DIR, source_stem=None):
...
def import_tools_to_wxo(
    ...
    for entry in tool_paths:
        ...
        # resolve() collapses relative/absolute and symlinked spellings of the
        # same file, which a plain string or Path comparison would miss.
        ...
        # One requirements file per TOOL, named after the tool rather than the
        # file, since that is the name the flow's tool node references. A file
        # holding several tools therefore yields several files.
        ...
        # The file that actually gets imported. By default the tool is extracted
        # out of its authoring notebook into src/flow_specs/ first, so the ADK
        # receives a plain module holding the tool and nothing else, rather than
        # the notebook's cells, SQL and UI widgets. Set extract=False to import
        # the authoring file verbatim.
        ...
        if extract:
            for name in declared:
                if tool_name and name != tool_name:
                    continue
                text = extract_tool_source(tool_path.read_text(), name)
                ...
                # One import per file; if several tools are declared the last
                # extraction wins, so extract them individually via (path, name).
                break

        # The ADK computes the tool's binding as `<path relative to cwd>:<func>`,
        # so running the CLI from the artefact directory with a BARE filename is
        # what produces the flat `fetch_url_data:fetch_url_data` binding that a
        # hand-written standalone tool gets. Passing `src/flow_specs/x.py` from
        # the repo root would instead bind `src.flow_specs.x:...`, a dotted path
        # whose packages do not exist server-side.
        run_cwd = None
        ...
        # Written next to the flow specs, and only rewritten when the parsed block
        ...
        # One `-k python` import ships the whole file, so the tools in it share a
        # single venv and pip gets one -r. Several tools each declaring their own
        # block means those blocks must agree; the union is what actually gets
        # installed, so a conflict between them surfaces at deploy, not here.
        dependencies = _dedupe(
            [spec for specs in declared.values() for spec in specs]
        )
        # Relative to run_cwd when one is set, so the -r path resolves from the
        # same directory the -f path does.
        def _arg(path):
            return path.name if run_cwd else str(path)

        if len(written) > 1:
            # The union is per-FILE rather than per-tool, so it is named for the
            # authoring file to keep it distinct from the per-tool files.
        ...
        proc = subprocess.run(
            command, capture_output=True, text=True, cwd=run_cwd
        )
